...
let calc_antinodes (y0, x0) (y1, x1) =
  let dy = Int.abs (y0 - y1) in
  let dx = Int.abs (x0 - x1) in

  let antinode1 =
    ( (if y0 < y1 then y0 - dy else y0 + dy),
      if x0 < x1 then x0 - dx else x0 + dx )
  in
  let antinode2 =
    ( (if y1 < y0 then y1 - dy else y1 + dy),
      if x1 < x0 then x1 - dx else x1 + dx )
  in
